...
def schema_upgrades() -> None:
    ...
    op.drop_constraint("sessions_session_location_id_fkey", "sessions", type_="foreignkey")
    ...
    op.drop_constraint("session_block_links_session_id_fkey", "session_block_links", type_="foreignkey")
    op.drop_constraint("session_block_links_block_id_fkey", "session_block_links", type_="foreignkey")
    op.drop_constraint("uq_session_block_links_session_block", "session_block_links", type_="unique")
    
    # Rename table
    op.rename_table("session_block_links", "block_links")
    
    # Recreate constraints with new table name
    op.create_unique_constraint("uq_block_links_block", "block_links", ["session_id", "block_id"])
    op.create_foreign_key("fk_block_links_session_id", "block_links", "sessions", ["session_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("fk_block_links_block_id", "block_links", "blocks", ["block_id"], ["id"], ondelete="CASCADE")
    
    # Step 5: Rename session_occurrences to occurrences
    # Drop foreign keys and constraints
    op.drop_constraint("session_occurrences_session_id_fkey", "session_occurrences", type_="foreignkey")
    op.drop_constraint("session_occurrences_block_id_fkey", "session_occurrences", type_="foreignkey")
    op.drop_constraint("ck_session_occurrences_time_valid", "session_occurrences", type_="check")
    ...
    op.create_check_constraint("ck_occurrences_time_valid", "occurrences", "starts_at < ends_at")
    op.create_foreign_key("fk_occurrences_session_id", "occurrences", "sessions", ["session_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("fk_occurrences_block_id", "occurrences", "blocks", ["block_id"], ["id"], ondelete="CASCADE")
    op.create_index("ix_occurrences_session_id", "occurrences", ["session_id"], unique=False)
    op.create_index("ix_occurrences_block_id", "occurrences", ["block_id"], unique=False)
    
    # Update comments to new table name
    op.execute("COMMENT ON COLUMN occurrences.auto_generated IS 'True if created by generate_occurrences, False if manually added'")
    op.execute("COMMENT ON COLUMN occurrences.block_id IS 'Which block this occurrence belongs to'")
    
    # Update attendance_records foreign key to occurrences
    op.drop_constraint("attendance_records_occurrence_id_fkey", "attendance_records", type_="foreignkey")
    op.create_foreign_key("fk_attendance_records_occurrence_id", "attendance_records", "occurrences", ["occurrence_id"], ["id"], ondelete="CASCADE")
    
    # Update attendance_audit_logs foreign key to occurrences
    op.drop_constraint("attendance_audit_logs_occurrence_id_fkey", "attendance_audit_logs", type_="foreignkey")
    op.create_foreign_key("fk_attendance_audit_logs_occurrence_id", "attendance_audit_logs", "occurrences", ["occurrence_id"], ["id"], ondelete="CASCADE")
    ...
    op.drop_constraint("attendance_records_child_id_fkey", "attendance_records", type_="foreignkey")
    op.drop_constraint("attendance_audit_logs_child_id_fkey", "attendance_audit_logs", type_="foreignkey")
    ...
    op.create_foreign_key("fk_students_caregiver_id", "students", "caregivers", ["caregiver_id"], ["id"])
    ...
    op.create_foreign_key("fk_signups_student_id", "signups", "students", ["student_id"], ["id"])
    op.create_foreign_key("fk_attendance_records_student_id", "attendance_records", "students", ["student_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("fk_attendance_audit_logs_student_id", "attendance_audit_logs", "students", ["student_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("fk_student_notes_student_id", "student_notes", "students", ["student_id"], ["id"], ondelete="CASCADE")
    
    # Update indexes
    op.drop_index("ix_child_notes_child_id", table_name="student_notes")
    op.create_index("ix_student_notes_student_id", "student_notes", ["student_id"], unique=False)
    ...
    op.drop_index("ix_attendance_audit_logs_child_id", table_name="attendance_audit_logs")
    op.create_index("ix_attendance_audit_logs_student_id", "attendance_audit_logs", ["student_id"], unique=False)
    
    # Update unique constraint in signups
    op.drop_constraint("uq_signups_session_child", "signups", type_="unique")
    op.create_unique_constraint("uq_signups_session_student", "signups", ["session_id", "student_id"])
    ...
    op.create_index("ix_block_links_block_id", "block_links", ["block_id"], unique=False)
    op.create_index("ix_block_links_session_id", "block_links", ["session_id"], unique=False)
...
def schema_downgrades() -> None:
    ...
    with op.batch_alter_table("sessions", schema=None) as batch_op:
        batch_op.add_column(sa.Column("waitlist", sa.Boolean(), nullable=False, server_default="false"))
    
    with op.batch_alter_table("students", schema=None) as batch_op:
        batch_op.add_column(sa.Column("needs_devices", sa.Boolean(), nullable=False, server_default="false"))
    
    with op.batch_alter_table("occurrences", schema=None) as batch_op:
        batch_op.add_column(sa.Column("auto_generated", sa.Boolean(), nullable=False, server_default="false"))
    
    with op.batch_alter_table("blocks", schema=None) as batch_op:
        batch_op.add_column(sa.Column("timezone", sa.String(length=64), nullable=False, server_default="Pacific/Auckland"))
    ...
    op.create_unique_constraint("uq_session_blocks_year_type", "blocks", ["year", "block_type"])
    
    # Revert students → children
    op.drop_constraint("uq_signups_session_student", "signups", type_="unique")
    op.create_unique_constraint("uq_signups_session_child", "signups", ["session_id", "child_id"])
    
    # Drop new foreign keys
    op.drop_constraint("fk_student_notes_student_id", "student_notes", type_="foreignkey")
    op.drop_constraint("fk_attendance_audit_logs_student_id", "attendance_audit_logs", type_="foreignkey")
    op.drop_constraint("fk_attendance_records_student_id", "attendance_records", type_="foreignkey")
    ...
    op.create_foreign_key("children_caregiver_id_fkey", "children", "caregivers", ["caregiver_id"], ["id"])
    op.create_foreign_key("child_notes_child_id_fkey", "child_notes", "children", ["child_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("attendance_audit_logs_child_id_fkey", "attendance_audit_logs", "children", ["child_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("attendance_records_child_id_fkey", "attendance_records", "children", ["child_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("signups_child_id_fkey", "signups", "children", ["child_id"], ["id"])
