...
def schema_upgrades() -> None:
    ...
    op.execute(
        "ALTER INDEX IF EXISTS ix_session_block_links_session_id RENAME TO ix_block_links_session_id"
    )
    op.execute(
        "ALTER INDEX IF EXISTS ix_session_block_links_block_id RENAME TO ix_block_links_block_id"
    )
    op.execute(
        "ALTER INDEX IF EXISTS ix_session_occurrences_session_id RENAME TO ix_occurrences_session_id"
    )
    op.execute(
        "ALTER INDEX IF EXISTS ix_session_occurrences_block_id RENAME TO ix_occurrences_block_id"
    )

    # STEP 6: Update unique constraints
    op.execute(
        "ALTER TABLE blocks DROP CONSTRAINT IF EXISTS uq_session_blocks_year_type"
    )
    op.execute(
        "ALTER TABLE blocks ADD CONSTRAINT uq_blocks_year_name UNIQUE (year, name)"
    )
    op.execute(
        "ALTER TABLE block_links DROP CONSTRAINT IF EXISTS uq_session_block_links_session_block"
    )
    op.execute(
        "ALTER TABLE block_links ADD CONSTRAINT uq_block_links_block UNIQUE (session_id, block_id)"
    )

    # STEP 7: Update check constraints (PostgreSQL auto-prefixes with old table name)
    op.execute(
        "ALTER TABLE occurrences DROP CONSTRAINT IF EXISTS ck_session_occurrences_ck_session_occurrences_time_valid"
    )
    op.execute(
        "ALTER TABLE occurrences ADD CONSTRAINT ck_occurrences_ck_occurrences_time_valid CHECK (starts_at < ends_at)"
    )

    # STEP 8: Rename foreign key constraints
    op.execute(
        "ALTER TABLE block_links DROP CONSTRAINT IF EXISTS session_block_links_session_id_fkey"
    )
    op.execute(
        "ALTER TABLE block_links DROP CONSTRAINT IF EXISTS session_block_links_block_id_fkey"
    )
    op.execute(
        "ALTER TABLE block_links ADD CONSTRAINT fk_block_links_session_id_sessions FOREIGN KEY (session_id) REFERENCES sessions(id) ON DELETE CASCADE"
    )
    op.execute(
        "ALTER TABLE block_links ADD CONSTRAINT fk_block_links_block_id_blocks FOREIGN KEY (block_id) REFERENCES blocks(id) ON DELETE CASCADE"
    )

    op.execute(
        "ALTER TABLE occurrences DROP CONSTRAINT IF EXISTS session_occurrences_session_id_fkey"
    )
    op.execute(
        "ALTER TABLE occurrences DROP CONSTRAINT IF EXISTS session_occurrences_block_id_fkey"
    )
    op.execute(
        "ALTER TABLE occurrences ADD CONSTRAINT fk_occurrences_session_id_sessions FOREIGN KEY (session_id) REFERENCES sessions(id) ON DELETE CASCADE"
    )
    op.execute(
        "ALTER TABLE occurrences ADD CONSTRAINT fk_occurrences_block_id_blocks FOREIGN KEY (block_id) REFERENCES blocks(id) ON DELETE CASCADE"
    )
    ...
    with op.batch_alter_table("attendance_records", schema=None) as batch_op:
        ...
        batch_op.drop_constraint(
            batch_op.f("attendance_records_occurrence_id_fkey"), type_="foreignkey"
        )
        batch_op.drop_constraint(
            batch_op.f("attendance_records_child_id_fkey"), type_="foreignkey"
        )
        batch_op.create_foreign_key(
            batch_op.f("fk_attendance_records_student_id_students"),
            "students",
            ["student_id"],
            ["id"],
            ondelete="CASCADE",
        ...
        batch_op.create_foreign_key(
            batch_op.f("fk_attendance_records_occurrence_id_occurrences"),
            "occurrences",
            ["occurrence_id"],
            ["id"],
            ondelete="CASCADE",
    ...
    with op.batch_alter_table("exclusion_dates", schema=None) as batch_op:
        ...
        batch_op.alter_column(
            "reason", existing_type=sa.VARCHAR(length=255), nullable=False
        )
    ...
    with op.batch_alter_table("sessions", schema=None) as batch_op:
        batch_op.add_column(sa.Column("sa_orm_sentinel", sa.Integer(), nullable=True))
        batch_op.drop_constraint(
            batch_op.f("sessions_session_location_id_fkey"), type_="foreignkey"
        )
        batch_op.create_foreign_key(
            batch_op.f("fk_sessions_location_id_locations"),
            "locations",
            ["location_id"],
            ["id"],
        )
    ...
    with op.batch_alter_table("signups", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "needs_devices", sa.Boolean(), server_default="false", nullable=False
            )
        )
        batch_op.add_column(sa.Column("sa_orm_sentinel", sa.Integer(), nullable=True))
        batch_op.drop_constraint(batch_op.f("uq_signups_session_child"), type_="unique")
        batch_op.create_unique_constraint(
            "uq_signups_session_student", ["session_id", "student_id"]
        )
        batch_op.drop_constraint(
            batch_op.f("signups_caregiver_id_fkey"), type_="foreignkey"
        )
        batch_op.drop_constraint(
            batch_op.f("signups_child_id_fkey"), type_="foreignkey"
        )
        batch_op.create_foreign_key(
            batch_op.f("fk_signups_student_id_students"),
            "students",
            ["student_id"],
            ["id"],
        )
...
def schema_downgrades() -> None:
    ...
    with op.batch_alter_table("signups", schema=None) as batch_op:
        ...
        batch_op.create_unique_constraint(
            "uq_signups_session_child", ["session_id", "child_id"]
        )
        batch_op.drop_constraint(
            batch_op.f("fk_signups_student_id_students"), type_="foreignkey"
        )
        batch_op.create_foreign_key(
            "signups_child_id_fkey", "children", ["child_id"], ["id"]
        )
        batch_op.create_foreign_key(
            "signups_caregiver_id_fkey", "caregivers", ["caregiver_id"], ["id"]
        )

    with op.batch_alter_table("sessions", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("waitlist", sa.Boolean(), server_default="false", nullable=False)
        )
        batch_op.drop_column("sa_orm_sentinel")
        batch_op.drop_constraint(
            batch_op.f("fk_sessions_location_id_locations"), type_="foreignkey"
        )
        batch_op.create_foreign_key(
            "sessions_session_location_id_fkey",
            "session_locations",
            ["session_location_id"],
            ["id"],
        )
    ...
    with op.batch_alter_table("exclusion_dates", schema=None) as batch_op:
        batch_op.alter_column(
            "reason", existing_type=sa.VARCHAR(length=255), nullable=True
        )
        batch_op.drop_column("sa_orm_sentinel")
    ...
    with op.batch_alter_table("attendance_records", schema=None) as batch_op:
        ...
        batch_op.create_index(
            "ix_attendance_records_child_id", ["child_id"], unique=False
        )
        batch_op.drop_constraint(
            batch_op.f("fk_attendance_records_occurrence_id_occurrences"),
            type_="foreignkey",
        ...
        batch_op.drop_constraint(
            batch_op.f("fk_attendance_records_student_id_students"), type_="foreignkey"
        )
        batch_op.create_foreign_key(
    ...
    """)
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_child_notes_child_id ON child_notes(child_id)"
    )
    ...
    """)
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_attendance_audit_logs_occurrence_id ON attendance_audit_logs(occurrence_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_attendance_audit_logs_child_id ON attendance_audit_logs(child_id)"
    )
